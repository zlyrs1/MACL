...
class BYOLTrainer:
    def __init__(self, args, device, labels_text, ignored_label):
        ...
        self.predictor = MLPHead(args.projection_size, args.mlp_hidden_size, args.projection_size, args.use_swish).to(
            device)
        ...
        train_pipeline = TrainPipeline(batch_size=args.batch_size, num_threads=args.num_threads, device_id=0,
                                       seed=args.seed, args=args)
        train_pipeline.build()
        if args.fastmode:
            self.train_loader = DALIClassificationIterator(pipelines=train_pipeline, auto_reset=True,
                                                           size=args.num_labels, last_batch_policy=LastBatchPolicy.DROP,
                                                           last_batch_padded=True)
        else:
            self.train_loader = DALIClassificationIterator(pipelines=train_pipeline, auto_reset=True,
                                                           size=args.H * args.W, last_batch_policy=LastBatchPolicy.DROP,
                                                           last_batch_padded=True)
    ...
    # 初始化online和target network的参数时，指定两者一致并且target的参数不会由loss更新
    @torch.no_grad()
    ...
    # 由batch_view_1和batch_view_2输入得到输出后计算损失函数
    ...
    def finetune(self, args, train_mask, test_mask, linear_protocol=True):
        ...
        train_loader = DataLoader(train_dataset, batch_size=args.batch_size_finetune, shuffle=True, drop_last=True,
        ...
        classifier = nn.Linear(in_features=args.hsi_channels[-1], out_features=self.class_num, bias=True).to(
            self.device)
        ...
        if linear_protocol:
            ...
            optimizer = Adam(params=classifier.parameters(), lr=args.lr_finetune_linear,
        ...
        for epoch in range(epochs):
            ...
            if linear_protocol:
                wandb.log({"loss_finetune_linear": LOSS})
            else:
                wandb.log({"loss_finetune_full": LOSS})
        ...
        if linear_protocol:
            wandb.log({"OA_finetune_linear": oa, "AA_finetune_linear": aa, "Kappa_finetune_linear": kappa})
        else:
            wandb.log({"OA_finetune_full": oa, "AA_finetune_full": aa, "Kappa_finetune_full": kappa})
        # return oa, aa, kappa
